...
* crero-yp is able to query back your CreRo instance to try to reach its crero-yp-api, which is used by the yp server to add a "more item informations..." link in your server entry in its listing, and this link can be used to learn more about your label ; its general label style as defined in radio_genres ; its list of artist as defined in artists -or- if your instance is an mp3-only (typically a playlist webradio/download service with and autobuildradiobase set and its free download tier operated by you containing only mp3 and no flac nor ogg audio files) and artists option undefined, the whole list of artist provided by the media tier.  Also, each album, for each artist (sorted by "streaming" and "downloadable" categories) ; for each album, the whole tracklist. More to come. 
