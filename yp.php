<?php
if (file_exists('./.timestamp')&&!is_dir('./.timestamp')){
	$tss=file_get_contents('./.timestamp');
	if ($tss!==false){
		$ts=unserialize($tss);
		
		if ((floatval($ts)-microtime(true))<4){
			sleep(4);
		}
...
file_put_contents('./.timestamp', serialize(microtime(true)));
